...
class Settings(BaseModel):
    ...
    institution: str = "Universidade Estadual de Goiás"
    unit: str = "Unidade Universitária de Ceres"
    course_name: str = "Sistemas para Internet"
    student_name: str = ""
    generated_root: Path = ROOT / "data" / "generated"
...
def load_settings(root: Path = ROOT) -> Settings:
    ...
    try:
        settings = Settings(
            ...
            astra_model=values.get("ASTRA_MODEL") or "gpt-5-mini",
            ...
            institution=values.get("INSTITUTION") or "Universidade Estadual de Goiás",
            unit=values.get("UNIT") or "Unidade Universitária de Ceres",
            course_name=values.get("COURSE_NAME") or "Sistemas para Internet",
            student_name=values.get("STUDENT_NAME") or "",
            generated_root=path("GENERATED_ROOT", "data/generated"),
            drive_responses_folder_id=values.get("DRIVE_RESPONSES_FOLDER_ID") or "",
            drive_responses_folder_name=values.get("DRIVE_RESPONSES_FOLDER_NAME")
            or "Classroom Agent - Respostas",
